...

{
	...
	double sx, so, S;//possibility whether agent can see defector defects or see an agent violates the metanorm;
...
double BAdaptiveLearning(int x)
{

	Player[x].HMinDS = (4 * T) + (4 * (G - 1) * P);
	Player[x].HMaxDS = (4 * T);
	if (Player[x].DS < 0)
	{
		Player[x].HMinDS = min(Player[x].HMinDS, Player[x].DS);
		Player[x].factorB = Player[x].DS / Player[x].HMinDS;
	}
	else
	{
		if (Player[x].DS > 0)
		{
			Player[x].HMaxDS = max(Player[x].HMaxDS, Player[x].DS);
			Player[x].factorB = Player[x].DS / Player[x].HMaxDS;
		}
		else
		{
			Player[x].factorB = 0;
			Player[x].HMaxDS = max(Player[x].HMaxDS, 0.0);
		}
	}
	Player[x].rb = LS * Player[x].factorB;
	return fabs(Player[x].rb);
